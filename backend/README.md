...
`gemini-2.0-flash`), `GOOGLE_RAG_MODEL` (default `gemini-2.0-flash`),
`DATABASE_URL` (Neon/PostgreSQL), `QDRANT_URL`, and `QDRANT_API_KEY`.
...
Document questions are answered directly with Gemini using retrieved document
context; they do not invoke the Groq CrewAI workflow or critic.
